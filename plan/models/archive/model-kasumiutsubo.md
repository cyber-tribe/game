> **実装済み。** `tools/models/monsters.py`に`build_kasumiutsubo()`/
> `kasumiutsubo_animations()`を追加し、`MONSTERS`に登録した
> (`tools/venv/bin/python tools/build_models.py kasumiutsubo`で
> `public/models/kasumiutsubo.glb`を書き出し)。
>
> **骨格は計画書どおり`tsubute`と同じ関節構成をベースにした。**
> 未決事項にあった「アーマチュアの流用可否」は、関節を頭からしっぽへ
> 大きく引き伸ばし、高さを大きく削ることで問題なく流用できることを
> 確認した。周囲に溶け込む平たく低いウツボのシルエットにした。
>
> **「存在感そのものが薄れた」由来を、`wasuremizuchi`よりさらに彩度を
> 落とした配色と、薄く小さな目で表現した。**
>
> **配色は計画書どおり、第二地方(忘れ潮の湿地)の霧と水を思わせる
> 灰みがかった水色・青緑系。**
>
> **三角形数は4,728**(既存モデルの目安1,800〜7,500の範囲内)。
>
> **アニメーションは`tsubute_animations()`を流用せず、新規に
> キーフレームを書いた。** `attack`は気配を消していた分、飛び出す一撃を
> 鋭く速い動きにした(クリップ構成・ボーン名は変えていない)。
>
> **見た目の確認は`tools/preview/kasumiutsubo.png`のCyclesレンダーで
> 行った。** 裏返り・体積ゼロ・パーツの誤結合といった破綻はない。
> `tools/compare_models.mjs`で全モデル(70個)を再ビルドして構造(頂点数・
> クリップ名・外形)を突き合わせ、一致することも確認した。
>
> **`src/entities/species.ts`の`kasumiutsubo.model`を`"tsubute"`→
> `"kasumiutsubo"`に変更した。** `kasumiutsubo`は

> **実装済み。** `src/view/assets.ts`の`toToonMaterial()`にリムライトを、
> `src/view/actorView.ts`に接地影を追加した。
>
> **リムライトは計画書どおりの実装で、`totalEmissiveRadiance`への
> フレネル項加算。** ただし1点、計画書の想定と違う対応が必要だった。
> `toToonMaterial()`は`Assets.load()`のtraverse内で**壁・床・罠・
> アイテムなど非キャラクターの静的な形状にも使われている**ため、無条件で
> リムライトを注入すると計画書の対象外(「壁・床など非キャラクターへの
> リムライト適用」)に反してしまう。`toToonMaterial(source, withRim)`に
> `withRim`引数を足し、呼び出し側で`mesh.isSkinnedMesh`(輪郭線の要否と
> 同じ判定)を渡すことでキャラクターだけに絞った。
>
> **接地影は計画書どおり、コード生成の放射状グラデーション板。**
> ジオメトリ・マテリアル・テクスチャは全アクター共有にした
> (計画書は「dispose()でジオメトリを破棄する」としていたが、
> ジオメトリに個体差が無い[サイズはmesh.scaleで表現する]設計にしたため、
> 破棄する理由が無い。むしろ共有物を1インスタンスのdispose()で破棄すると
> 他の生存インスタンスを壊すため、破棄しないほうが正しい。テクスチャ・
> マテリアルと同じ扱いにした)。
>
> **サイズはモデルのバウンディングボックス(XZ)の大きい方の辺×1.15。**
> `ActorView`のコンストラクタで、`setPosition`・回転を乗せる**前**に
> `Box3.setFromObject(root)`を測ることで、モデル本来のfootprintが
> 正しく取れることを確認した。
>
> **`document`が無いテスト環境(vitestの`environment: "node"`)への対応
> として、テクスチャ生成を`typeof document === "undefined"`で
> ガードした。** これにより`tests/actor-view-move-sync.test.ts`
> (DOM無しで`new ActorView(...)`を直接呼ぶ既存テスト)が無改修で
> 通ることを確認した(影は単に出ないだけで、エラーにはならない)。
>
> **確認は`tools/playtest.mjs`のヘッドレスブラウザ実行で行った。**
> プレイヤー・モンスターの足元に丸い接地影が付き、移動にも追従すること、
> 被弾フラッシュ・目の発光が従来どおり機能することをスクリーンショットで
> 確認した。
>
> `npx tsc --noEmit`・`npx vitest run`(1350/1350)・`npm run build`は
> すべてgreen。未決事項(リム色の気分連動、各パラメータの最終値、
> 接地影の濃さの微調整)は初期実装の値のまま持ち越している。

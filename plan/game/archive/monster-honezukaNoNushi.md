...
> "honezukaNoNushi"`エントリを確認し、本文書のデータ表(AI/maxHp/atk/def/
> exp/weight/地方ボス)がすべて一致することを検証した。差分なし。本文書は
> コードを伴わない記録用の文書のため、追加の実装は行わない。

# ホネヅカのぬし(honezukaNoNushi)のモンスター実装

`src/entities/species.ts`にすでに実装済みの内容を、正式なplanとして
書き起こす(3Dモデルは`plan/models/archive/model-honezukaNoNushi.md`で別途扱う)。

...
> "houshitobi"`エントリを確認し、本文書のデータ表(AI/maxHp/atk/def/exp/
> weight/出現階/range/睡眠付与)がすべて一致することを検証した。差分
> なし。本文書はコードを伴わない記録用の文書のため、追加の実装は行わない。

# ほうしとび(houshitobi)のモンスター実装

`src/entities/species.ts`にすでに実装済みの内容を、正式なplanとして
書き起こす(3Dモデルは`plan/models/archive/model-houshitobi.md`で別途扱う)。

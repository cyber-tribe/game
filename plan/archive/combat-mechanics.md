...
> **実装済み。** `src/systems/combat.ts`(`MAX_CRIT_RATE` 上限20%)・
> `src/game.ts`(`attack()` の不意打ち判定・身構えのダメージ軽減)・
> `src/entities/player.ts`(`PlayerState.guarding`)。
> テストは `tests/combat-mechanics.test.ts`。
>
> 実装時の決定:
> - 不意打ちは「プレイヤーの攻撃」限定にせず、**攻撃者を問わず
>   `target.kind === "monster" && !target.aware` の全攻撃**(仲間の攻撃も
>   含む)に適用した。同じ `attack()` を通る既存構造にそのまま乗せられ、
>   「気づかれていない相手を殴ると必ず急所に入る」という理屈にも合うため
> - 身構えは、想定より単純化した: 「足踏み以外の行動を選んだら解ける」と
>   いう仕様案は採らず、**素直に「次に被弾するまで持続する」**とした
>   (原文の「次に被弾するまでの間だけ」を文字どおりに実装。移動や攻撃を
>   挟んでも解除されない)
> - 身構えの2割軽減は `attack()` 経由のダメージ(モンスター・仲間からの
>   通常攻撃)にのみ適用し、罠・毒・満腹度切れのような非攻撃ダメージは
>   対象外にした(「被弾」＝攻撃を受けること、という語感に合わせた)
> - 回避(ドッジ)は仕様どおり共通ルール化していない

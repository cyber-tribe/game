...
# 斑点の実測(全高 140mm 換算): 等価直径の中央 1.4〜1.9mm、被覆 3〜7%。
# しきい値 118 / 面積 40px² では 1体あたり 20〜30 個しか拾えず、設定画の
# 「全身の細かなまだら」にならなかった(実測では 80〜105 個ある)
SPOT_LUM = 112                 # 明色斑点の下限(体の中央値は 86 前後)
SPOT_MIN = 2                   # 斑点の最小面積(シートpx²)
SPOT_MAX = 95                  # 最大面積。これ以上は腹などの大きな色面
SPOT_FILL = 0.34               # 外接箱に対する充填率の下限(細長い影を除く)
...
# ---- 色。設定画の絵の中の実測を、モデルの体色 (0.335,0.278,0.296) との比で写す。
#      基調(113,100,118) / 斑点(145,131,130) → 斑点は基調の 1.6 倍で、
#      明るいほど青が抜ける(handbook 4-63)。
#      以前は 1.9 倍 × alpha 0.38 = 実効 1.3 倍で、96px では完全に消えていた
LINE_RGB = (0.13, 0.10, 0.13)
LID_RGB = (0.61, 0.51, 0.51)
SPOT_RGB = (0.58, 0.50, 0.50)
SPOT_ALPHA = 0.80              # 設定画の斑点は輪郭が柔らかいが、色は確かに違う

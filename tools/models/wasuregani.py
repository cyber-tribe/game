...
GAIN = {
    ...
    "limb": 1.14, "limbdark": 1.20, "nail": 1.06, "nailshade": 1.06,
...
# **固定指は下の顎。** `_claw_body` の up は必ず +z を向くので、r_out が
# 上・r_in が下。指の区間で r_out > r_in にしていた版は、掌と連続する太い
# 指が**上**に来ていて、設定画と上下が逆だった。設定画の固定指は
#   ・下へ厚い楔(掌からそのまま続く)
#   ・上は平らな噛み合わせの面
# で、細く長い可動指がその上から噛み込む。
CLAW_OUT = (0.054, 0.104, 0.118, 0.096, 0.052, 0.026, 0.016, 0.005)
CLAW_IN = (0.050, 0.092, 0.104, 0.098, 0.086, 0.062, 0.036, 0.005)
CLAW_SIDE = (0.046, 0.076, 0.084, 0.078, 0.062, 0.046, 0.028, 0.005)
CLAW_SEG = 12
...
# **固定指の上に置く。** 設定画の可動指は掌の前上の関節から出て、
# 固定指より **5割長く・半分の太さ**の細い鉤として上から噛み込む。
# 前の版は固定指の下・内側に置いていて、太い方が上に来ていた。
# 固定指の**噛み合わせの面(上側)より上**に、隙間を保って置く。面の高さは
# 中心線 + r_out で、指の区間では 0.180 → 0.037 と下がっていく。ここへ
# 重ねて置いた版は、細い可動指が太い固定指の中に丸ごと埋まって消えた。
# 長さは固定指の 1.33 倍(立ち絵の実測 73px 対 56px)。
# 中ほどを持ち上げて、噛み合わせの**隙間**を開ける。固定指の面に沿わせた
# 版は2本が密着し、閉じきった1枚の刃に見えた ―― 歯が入る隙間が無いと
# 「挟む」が読めない。先だけ寄せて閉じる。
MOV_SPINE = ((0.276, -0.448, 0.216), (0.282, -0.486, 0.196),
             (0.264, -0.518, 0.138), (0.224, -0.542, 0.062))
MOV_OUT = (0.034, 0.028, 0.018, 0.004)
MOV_IN = (0.030, 0.022, 0.014, 0.004)
MOV_SIDE = (0.026, 0.021, 0.014, 0.004)
MOV_SEG = 10
# 噛み合わせ。固定指の内側の面に2つ、可動指の内側に1つ。
# 固定指の歯は**上**(可動指の側)、可動指の歯は**下**。前の版は逆だった。
# u は **CLAW_SPINE 全体**の媒介変数なので、固定指の範囲(FIXED_FROM=0.52
# より先)に入れる。0.47 に置いた版は掌の上に歯が1つ載っていた。
BITE_FIXED = ((0.62, 0.86), (0.76, 0.74), (0.87, 0.60))
BITE_MOV = ((0.38, 0.86), (0.60, 0.70))
# **左右を鏡像にしない。**
...
#   v=0.00 横 / v=0.25 **背** / v=0.50 横 / v=0.75 **噛み合わせ側**
# になる。稜線を v=0 に、鋸歯を v=0.5 に置いた最初の版は、どちらも刃の
# **側面**に出ていて、背にも噛み合わせにも何も無かった。
#
# **固定指と可動指では噛み合わせの向きが逆**(固定指は上、可動指は下)。
# テクスチャを2枚にせず、固定指側だけ UV の v を反転して同じ絵を使う
# ―― 反転すれば、絵の「噛み合わせ側」がその部品の実際の噛み合わせ側へ、
# 「背」が実際の背へ、同時に回る。
CLAW_TEX = 128
...
def _claw_body(name, spine, r_out, r_in, r_side, seg, side, k=1.0, chip=1.0,
    ...
    for poly in me.polygons:
        for li in poly.loop_indices:
            ...
            i, j = divmod(vi, seg)
            v = j / seg
            uvl[li].uv = (u0 + (u1 - u0) * i / max(1, nring - 1),
                          1.0 - v if vflip else v)

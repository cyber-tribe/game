...
AKUBI_TORSO_RINGS = [
    (0.0224, 0.0548, 0.0520, 0.0, 0.0187),
    (0.0298, 0.0576, 0.0547, 0.0, 0.0195),
    (0.0371, 0.0567, 0.0538, 0.0, 0.0195),
    (0.0445, 0.0519, 0.0493, 0.0, 0.0188),  # (est)
    (0.0518, 0.0410, 0.0389, 0.0, 0.0084),  # (est)
    (0.0592, 0.0381, 0.0362, 0.0, 0.0048),  # (est)
    (0.0665, 0.0338, 0.0333, 0.0, 0.0038),
    (0.0739, 0.0290, 0.0267, 0.0, -0.0019),  # 腰の少し上のくびれ(waist)
    (0.0812, 0.0390, 0.0348, 0.0, -0.0033),
    (0.0886, 0.0414, 0.0381, 0.0, -0.0105),
    (0.0959, 0.0410, 0.0386, 0.0, -0.0157),
    # z=0.1033は実測では胸(rx=0.0386)よりむしろ奥行きが深い(ry=0.0457)
    # 値になっており、胸→頭の間に首のくびれが一切無かった(側面が
    # 「胸から垂直に立ち上がる壁」に見える指摘の主因)。ここだけは
    # 実測値のまま繋がず、頭と胸の間に本物のくびれを彫る。
    # 最初は2段(胸→くびれ→頭)で彫ったところ、間隔が狭すぎて
    # 「マント/えりのような硬い段差」に見えた。前後にもう1段ずつ
    # 挟んで勾配をなだらかにする
    (0.0996, 0.0375, 0.0365, 0.0, -0.0120),  # 首への遷移(下)
    (0.1033, 0.0335, 0.0335, 0.0, -0.0095),  # 首のくびれ(実測値を離れて彫る)
    (0.1070, 0.0390, 0.0400, 0.0, -0.0115),  # 首への遷移(上)
...
AKUBI_HALF = {
    ...
    # 無いという指摘)。付け根を実測よりわずかに外側・後方へ出し、
    # 胴の輪郭とのあいだに谷ができる余地を作る
    "legF.L": (0.033, -0.008, 0.085),
    "footF.L": (0.026, -0.018, 0.010),
    "legB.L": (0.038, 0.014, 0.046),
    "footB.L": (0.030, -0.006, 0.010),
    ...
    "tail6": (0.022, 0.0705, 0.0562),
    "tail7": (0.012, 0.0657, 0.0504),
}
...
def build_akubitokage():
    ...
    # 前回の値は付け根の直後(tail1)で0.036→0.0133までいきなり
    # 半分以下に落ちており、これが「早い段階から細くなり、ほぼ均一径の
    # 輪っかに見える」指摘の原因だった。tail1〜3あたりまで太さを保ち、
    # 巻き込みが始まるtail5以降だけ細くする
    # (先端tail7はsculpt_mergeのvoxel[0.0026]より細いと解像できず
    # 欠けが出るため、voxelよりわずかに太い0.0030を保つ)
    parts.append(C.curve_tube("akubi_tail", tail_pts,
                              [0.032, 0.026, 0.020, 0.014, 0.010, 0.007, 0.0050, 0.0030]))
    ...
    # ほぼ寄与しなくなっていた(指摘を受けて見直す)。IoUはこれ以上
    # 追わず、設定画の側面で背びれが実際にシルエットへ出ている
    # 大きさまで戻す
    _frill_keys = [
        (0.0314, 0.1106, 0.011), (0.0229, 0.0959, 0.018), (0.0315, 0.0812, 0.021),
        (0.0371, 0.0665, 0.016), (0.0473, 0.0518, 0.011), (0.0733, 0.0371, 0.006),
    ]

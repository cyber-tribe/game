...
# 複数の球を重ねて直接なぞれる。
# 胴・頭の断面(z, rx, ry, cx, cy)。reference-akubitokage-sheet.pngの
# 正面図・背面図・側面図から、****球を目視で調整するのではなく****
# 実際に輪郭をトレースして高さごとの横幅(正面図)・奥行き(側面図)を
# 픽셀計測し、AKUBI_HEIGHTへスケールした値。手順は
# plan/models/akubitokage-remake.md追記参照:
#   1. 三色(背景/前後/横)を色距離でしきい値化し、最大の連結成分を
#      胴体シルエットとして抽出(煙・案内破線・影は別の小さな成分になる)
#   2. 頭頂(z=1.0)〜接地(z=0.0)の行範囲を3面で突き合わせ(84〜86行/
#      231〜233行で一致、面ごとの誤差1〜2px)、高さの基準に採用
#   3. 各行について、正面図の横幅→rx、側面図の水平幅→depth(2*ry)を
#      読み取る。側面図はz<0.46で尾の巻きが胴と重なって奥行きが実際
#      より大きく出る(z=0.46でdepthが67px→110pxへ不連続に跳ぶ)ため、
#      その帯は「胴の前端(尾に隠されない側)」だけ実測し、奥行きは
#      前端が信頼できる帯(z=0.48〜0.95)のrx:depth比(平均0.95)から
#      推定した(下のrings内、コメント"(est)"の行)
# 尻(z最小)がrx=0.058で全身最大、頭(z最大付近)はrx=0.041どまりと、
# 「頭でなく尻がいちばん幅がある」がに股のトカゲ体型だと分かった
# (旧実装はhip 0.048 / head 0.044とほぼ同格にしていたが、実測はhipが
# headの1.4倍太い)。z=0.16(尻の底)より下は脚・尾の付け根で覆われる
# ため測っていない(脚・尾は別途curve_tubeで作る)
AKUBI_TORSO_RINGS = [
    # 設定画と正面/側面で並べ直した結果、次の2点が最優先の指摘として
    # 残った:
    # ① 正面が「四角い胴体」に見える ―― 実測rxをそのまま使うと腰
    #   (0.055〜0.058)が全身でいちばん太く、脇の下から胸にかけて
    #   絞りが無いまま腕がただの横の膨らみに乗っているように見える。
    #   設定画は逆で、腕・肩がいちばん幅を作り、胴そのものはもっと
    #   細い。ここは実測rxを全体的に絞り、幅の主役を腕(下記
    #   legF.L/legB.Lの位置)へ譲る
    # ② 側面の「顎の下から胸までがほぼ垂直な壁」―― 実測cyは腰から
    #   胸にかけていったん滑らかに後退するのに、胸(旧-0.0157)が
    #   首より前へ戻ってしまい、首で彫ったくびれの効果を相殺していた。
    #   前面(cy-ry)が頭→首→胸で単調に後退するようcyを引き直す
    (0.0224, 0.0480, 0.0520, 0.0, 0.0187),
    (0.0298, 0.0500, 0.0547, 0.0, 0.0195),
    (0.0371, 0.0460, 0.0538, 0.0, 0.0195),
    (0.0445, 0.0390, 0.0493, 0.0, 0.0188),  # (est)
    (0.0518, 0.0340, 0.0389, 0.0, 0.0084),  # (est)
    (0.0592, 0.0320, 0.0362, 0.0, 0.0048),  # (est)
    (0.0665, 0.0300, 0.0333, 0.0, 0.0038),
    (0.0739, 0.0280, 0.0267, 0.0, -0.0019),  # 腰の少し上のくびれ(waist)
    (0.0812, 0.0330, 0.0348, 0.0, 0.0048),
    (0.0886, 0.0350, 0.0381, 0.0, 0.0131),
    (0.0959, 0.0350, 0.0386, 0.0, 0.0186),  # 胸(前面をここで一気に後退させる)
    (0.0996, 0.0330, 0.0365, 0.0, 0.0115),  # 首への遷移(下)
    (0.1033, 0.0310, 0.0335, 0.0, 0.0035),  # 首のくびれ
    (0.1070, 0.0350, 0.0400, 0.0, 0.0000),  # 首への遷移(上)
    (0.1106, 0.0450, 0.0457, 0.0, -0.0143),
    (0.1180, 0.0333, 0.0419, 0.0, -0.0181),
    (0.1253, 0.0281, 0.0343, 0.0, -0.0200),  # 鼻先(いちばん前へ出る)
    (0.1327, 0.0200, 0.0310, 0.0, -0.0143),
    (0.1400, 0.0019, 0.0100, 0.0, -0.0143),  # 頭頂(点に収束)
]

AKUBI_HALF = {
    # ロフト(上のAKUBI_TORSO_RINGS)の代表点を関節として使う。
    # 位置はロフトのリングそのものから拾っているので、彫刻用の半径
    # (旧AKUBI_SCULPT_R)は不要になった
    "hip": (0.000, 0.0187, 0.0224),     # 尻(ロフト最下段=尾・後脚の付け根)
    "chest": (0.000, -0.0157, 0.0959),  # 肩(前脚の付け根)
    "head": (0.000, -0.0143, 0.1106),   # 頭(いちばん高く張り出す段)
    "snout": (0.000, -0.0200, 0.1253),  # 鼻先(いちばん前に出る段)
    # あくびで開閉する下あご。骨だけ持ち肉付けはしない(下記参照)。
    # 実測ロフトへの作り直しで鼻先(snout)の位置が大きく変わったのに
    # ここは旧胴(PR#1063)の値のまま残っており、鼻先より前に飛び出して
    # 顔の前に別の塊が浮いて見える事故になっていた(変形テストで発見)。
    # 旧snout(-0.055,0.086)→旧jaw(-0.046,0.076)の相対オフセット
    # (+0.009, -0.010、支点からの距離0.0135)を新しいsnoutにそのまま
    # 適用し、あご開閉のてこの効き方は変えずに位置だけ引き直す
    "jaw": (0.000, -0.011, 0.1153),
    # 脚は正面図(横幅・接地位置)と側面図(奥行き・付け根の高さ)を
    # 実測。前脚は側面図で細く付け根が高い(肩の高さ)、後脚は逆に
    # 付け根が低く(尻の高さ)、腹の下で大きな塊(もも)になっている。
    # 実測位置のままだと付け根が胴の輪郭の内側に収まってしまい、
    # 腕・ももが胴に溶け込んで見える(「腕と胴の谷」「腿と腹の谷」が
    # 無いという指摘)。さらに、設定画は腕・肩が全身でいちばん幅を
    # 作り、胴自体はそれより細い(逆に今の胴は腰が全身最大幅で
    # 「頭の下に四角い箱が付いた」ように見える、との指摘)。上の
    # AKUBI_TORSO_RINGSを絞ったのに合わせ、付け根をさらに外側へ出し、
    # 腕・ももが胴より明確に幅の主役になるようにする
    "legF.L": (0.042, -0.006, 0.087),
    "footF.L": (0.028, -0.018, 0.010),
    "legB.L": (0.044, 0.014, 0.046),
    "footB.L": (0.032, -0.006, 0.010),
    # 尾: 側面図の巻きをそのまま中心線としてなぞった(7点)。太さも
    # 各点の実測(下のtail_pts/radii参照)。Claudeの解釈による「煙の
    # ような渦」ではなく、線そのものをトレースした結果として先端が
    # 内側へ丸まる
    "tail1": (0.010, 0.0229, 0.0657),
    "tail2": (0.022, 0.0438, 0.0438),
    "tail3": (0.032, 0.0657, 0.0276),
    "tail4": (0.038, 0.0867, 0.0200),
    "tail5": (0.032, 0.0962, 0.0343),
    # tail6/tail7は指摘を受けて引き直した。実測どおりの位置だと
    # 渦の中心にできる穴が設定画よりかなり大きく開いて見えた
    # (設定画は穴がごく小さい)。中心線の通過点を渦の内側へ
    # 寄せ、先端が中心近くまで巻き込むようにする
    "tail6": (0.026, 0.0680, 0.0480),
    "tail7": (0.022, 0.0640, 0.0400),
}
...
AKUBI_SHEET = {
    ...
    # おなか(薄い影)。実測(#d7cbc8)は暖色寄りの淡いグレーだが、
    # 実機で鱗の濃淡(screen-blend)と重なると発光する白い球のように
    # 見えるとの指摘を受けた。設定画は「薄紫の腹」なので、明るさを
    # 少し落とし紫寄りに振る
...
# 実測ロフト(AKUBI_TORSO_RINGS)への作り直しで胴自体が一回り小さく
# 引き締まったのに、ここは旧・球積み上げ版の胴に合わせた大きな値の
# ままだった。実機ターンテーブルで確認すると、淡色の楕円体が下半身の
# ほぼ全体を覆って白飛びして見えた。新しい膨らみ(半径0.018)に合わせて
# 縮め直した(その後、正面が平面的すぎる指摘を受けて膨らみ自体を
# 半径0.016→0.024へ再拡大したため、ここも合わせて広げ直す)
AKUBI_BELLY_CENTER = Vector((0.0, -0.014, 0.022))
AKUBI_BELLY_RADII = Vector((0.028, 0.026, 0.024))
...
def _akubi_scale_groups():
    ...
    # 領域の中心・半径は、彫刻用の球(旧AKUBI_SCULPT_R)ではなく新しい
    # AKUBI_TORSO_RINGSの該当リングから拾う(hip/chestは関節と同じ高さの
    # リング、midbackは胴の中央あたりのリング)
    hip = Vector(AKUBI_HALF["hip"])
    hip_r = 0.055  # z=0.0224のリング(rx=0.0548)
    groups.append((hip, hip_r, _akubi_scale_sphere_anchors(hip, hip_r, _akubi_scale_bands(13, 15))))

    mid = Vector((0.0, 0.0038, 0.0665))  # z=0.0665のリング(胴の中央あたり)
    mid_r = 0.034
    groups.append((mid, mid_r, _akubi_scale_sphere_anchors(mid, mid_r, _akubi_scale_bands(11, 14))))

    chest = Vector(AKUBI_HALF["chest"])
    chest_r = 0.040  # z=0.0959のリング(rx=0.0410)
...
def build_akubitokage():
    ...
    # 胴+頭は、球を目視で重ねるのではなく設定画から実測した断面
    # (AKUBI_TORSO_RINGS)をそのままロフトする。garudoの頭+胴
    # (HEAD_RINGS/g_torso)と同じ手法の共有版
    parts.append(C.loft("akubi_torso", AKUBI_TORSO_RINGS, segments=20))
    ...
    # 足先の肉球でまた少し太くなる(単調な先細りではない)。
    #
    # 以前はここで付け根に「肩・尻」の別体の球を足して胴との継ぎ目を
    # 埋めていたが、それこそが「腕と胴の谷」「腿と腹の谷」を消して
    # 肉塊に見せていた原因だった(指摘を受けて撤回)。代わりに
    # AKUBI_HALFの付け根位置そのものを胴の輪郭より少し外側・後方へ
    # 出し(上記参照)、付け根そのものの半径を太くして「独立した腕・
    # ももの塊」にする。埋める球を足さないので、胴の輪郭が凹む位置に
    # そのまま谷が残る
    for side in (-1.0, 1.0):
    ...
    # 設定画は根元が非常に太く、渦を巻き始める終盤だけ細くなる
    # (最初の区間でいきなり半分以下まで細くなる先細りではない)。
    # 付け根(hip)の半径を0.038まで太くしたところ、大腿(legB.L、
    # x=0.044で胴の外へ張り出す太い塊)と同じ場所を取り合って融合し、
    # 尾の渦そのものが消えてしまう事故になった(以前の「もう1つの頭」
    # と同種の事故)。付け根はhip関節そのものの半径(0.032、大腿とは
    # 別の場所で競合しない実績値)へ戻し、tail1〜3の途中区間だけを
    # 太くして「早く細くなりすぎ」を解消する
    # (先端tail7はsculpt_mergeのvoxel[0.0026]より細いと解像できず
    # 欠けが出るため、voxelよりわずかに太い0.0030を保つ)
    parts.append(C.curve_tube("akubi_tail", tail_pts,
                              [0.032, 0.027, 0.022, 0.016, 0.011, 0.007, 0.0050, 0.0030]))
    ...
    for side in (-1.0, 1.0):
        ...
        # 頬: 半目の線の真下・やや後ろに置く。z=0.104は上で彫った首の
        # くびれ(z=0.1033、rx=0.028)のすぐ下なので、頬を大きく張らせる
        # ほど首との谷がはっきり出る。半径を大きくして実際に張らせる
        cheek_c = Vector((0.036 * side, -0.012, 0.104))
        parts.append(C.uv_sphere(f"akubi_cheek{side}", tuple(cheek_c), 0.021,
                                 segments=10, rings=8, scale=(0.9, 1.0, 0.6)))
    ...
    # 球を並べ、独立した棘(△△△△)ではなく体表のうねり(〜∿〜)に
    # 見せる。高さは頭寄りで低く→背の中央でいちばん高く→腰へ向けて
    # 消える山なりにする(設定画には角のある部品が無いため、
    # silhouette-hard-surface-parts.mdの「最低1つ」は今回見送る)。
    #
    # 手置きの球をそのまま並べただけでは、隣どうしの間隔がY方向の
    # 半径より狭くても理屈上重なるだけで、球の断面がY方向の端へ向けて
    # 窄まるぶん規則的な浅いくびれができ、Clay(単色)レンダーで
    # 「団子が並んだ背中」に見えることが分かっている。制御点を密に
    # 補間し直し、間隔をvoxel(0.0026)に対して十分狭くしてくびれを潰す
    # 一度、シルエットIoU比較で外へ張り出しすぎと分かって縮めた
    # (先端0.022→0.007まで)が、縮めすぎて側面のキャラクター性に
    # ほぼ寄与しなくなっていた。前回0.011〜0.021まで戻したが、まだ
    # 「シルエットからほぼ消えている」と指摘され、設定画程度まで
    # 出して構わないとのことなので、旧・最大時に近い大きさまで戻す。
    #
    # y0は以前、胴のリングの背面(cy+ry)からそのまま拾っていたが、
    # 指摘②で胸(z=0.0959)のcyを大きく後退させた結果、その1点だけ
    # 背面が0.057まで跳ね上がり、球の伸び(1.6倍)も足すと背中に
    # 三角の「帆」が立つ事故になった。以降はリングの値をそのまま
    # 使わず、なだらかな山なりになるよう手で置き直す
    _frill_keys = [
        (0.0320, 0.1106, 0.013), (0.0380, 0.0959, 0.020), (0.0400, 0.0812, 0.022),
        (0.0380, 0.0665, 0.017), (0.0450, 0.0518, 0.012), (0.0700, 0.0371, 0.007),
    ]
    ...
    # 半径を大きくした分、補間数も増やして「団子」対策の重なりを保つ
    _frill_n = 26
    for i in range(_frill_n):
        y, z, r = _frill_lerp(i / (_frill_n - 1))
        parts.append(C.uv_sphere(f"akubi_frill{i}", (0.0, y, z), r,
                                 segments=10, rings=7, scale=(0.68, 1.6, 0.42)))

    # 腹の膨らみ。新しい胴のロフトはすでに正面図・側面図の実測どおり
    # 腰の断面が大きく前へ張り出す形になっているため、旧版のような
    # 大きな球は不要になった……はずだったが、実際には正面が
    # 「大きな平面」に見える指摘を受けた。ロフトの前への張り出しだけ
    # では丸い腹の量感が足りないので、球そのものを大きく戻す。
    # 球1個だけだと中心対称で「貼り付いたこぶ」に見えるため、少し
    # 下・前へずらした2個目を重ねて、重力で下へ垂れているように
    # 上より下のほうが張り出すいびつな形にする
    parts.append(C.uv_sphere("akubi_belly_bulge",
                             (0.0, AKUBI_BELLY_CENTER.y, AKUBI_BELLY_CENTER.z),
                             0.024, segments=14, rings=10, scale=(0.95, 0.9, 0.85)))
    parts.append(C.uv_sphere("akubi_belly_droop",
                             (0.0, AKUBI_BELLY_CENTER.y - 0.005, AKUBI_BELLY_CENTER.z - 0.008),
                             0.020, segments=12, rings=9, scale=(0.9, 0.95, 0.7)))

    # 入力は自己交差の無い閉じたプリミティブ(ロフト・球・curve_tube)
    # だけなのでclean_input=Trueで近道する。既定Falseの前段SMOOTH
    # リメッシュは「交差しているだけで未融合」と判定した大きめの部品を
    # 削ることがあり、実測で鼻先の付け根に不自然な穴(凹み)ができた。
    ...
    head_r = 0.044  # AKUBI_TORSO_RINGSのz=0.1106のリング(rx=0.0414/ry=0.0457)相当
    ...
    snout_r = 0.031  # AKUBI_TORSO_RINGSのz=0.1253のリング(rx=0.0281/ry=0.0343)相当
    ...
    tail_tip = Vector(AKUBI_HALF["tail7"])
    ...
    def akubi_color(p, n):
        ...
        # 判別できる太さまで戻した。それでもまだ「半目がほぼ読めない」
        # 指摘が続いたため、さらに太くする(半目は設定画でいちばん
        # 重要な識別記号なので、多少太すぎるくらいを許容する)
        ed = min(seg_dist(q, a, b) for a, b in zip(eye_pts, eye_pts[1:]))
        if ed < 0.014 and n.y < -0.2:
            return dark
    ...
    # あくびの煙。以前は2個の大きな完全な球で、設定画の「ふわっとした
    # 煙」というより硬い泡のように見えていた(指摘を受けて見直す)。
    # 1つの房を大きさの違う3個の小さな球の集まりに置き換え、輪郭を
    # 完全な円にしない(kinokootokoの胞子と同じ、primitiveを貼るだけの
    # 安全な手法のまま、粒を増やして雲らしくする)
    smoke_mat = C.make_material("akubi_smoke", (0.75, 0.70, 0.82), roughness=0.5, emission=0.15)
    smoke_puffs = [
        (0.052, -0.027, 0.114, 0.0075), (0.060, -0.019, 0.120, 0.0065),
        (0.054, -0.017, 0.125, 0.0055),
        (0.046, 0.008, 0.130, 0.0065), (0.053, 0.004, 0.135, 0.0055),
        (0.048, 0.011, 0.139, 0.0045),
    ]
    for i, (x, y, z, r) in enumerate(smoke_puffs):
        smoke = C.uv_sphere(f"akubi_smoke{i}", (x, y, z), r, segments=8, rings=6)

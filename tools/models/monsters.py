...
AKUBI_TORSO_RINGS = [
    ...
    (0.1033, 0.0386, 0.0457, 0.0, -0.0114),
    (0.1106, 0.0414, 0.0457, 0.0, -0.0143),
...
AKUBI_HALF = {
    ...
    "legF.L": (0.026, -0.010, 0.083),
    "footF.L": (0.024, -0.020, 0.010),
    "legB.L": (0.030, 0.012, 0.050),
    "footB.L": (0.026, -0.008, 0.010),
...
AKUBI_BELLY_RADII = Vector((0.020, 0.018, 0.018))
...
def build_akubitokage():
    ...
    # 半径は、シルエットIoU比較(正面/側面/背面と設定画マスクの重ね合わせ)
    # で背びれが設定画より外へ張り出しすぎていたのを見て縮めた
    # (先端0.022だとY方向の伸び1.9倍込みで背面輪郭が実測より4cm近く
    # 外側へ出ていた)
    _frill_keys = [
        (0.0314, 0.1106, 0.007), (0.0229, 0.0959, 0.012), (0.0315, 0.0812, 0.014),
        (0.0371, 0.0665, 0.011), (0.0473, 0.0518, 0.007), (0.0733, 0.0371, 0.004),
    ]
    ...
    # 半径を縮めたぶん、球どうしのY方向の重なりが足りず「団子」に
    # 戻らないよう補間数を増やす(伸び倍率も1.9→1.5へ下げて張り出しを
    # さらに抑える)
    _frill_n = 20
    for i in range(_frill_n):
        y, z, r = _frill_lerp(i / (_frill_n - 1))
        parts.append(C.uv_sphere(f"akubi_frill{i}", (0.0, y, z), r,
                                 segments=10, rings=7, scale=(0.68, 1.5, 0.42)))

...
BODY_LOOPS = [
    ...
    # 胸: 腹から立ち上がる。背中は垂直
    (0.075, -0.001, 0.029, 0.029, 0.031, 0.0, "chest"),
    (0.083, -0.003, 0.027, 0.027, 0.029, 0.0, "shoulder"),    # 正面のくびれ(最小幅0.058)
    # 喉〜顎: 「首」は無い。前縁が斜めに滑り出し、横幅は頬へ広がる。
    # 正面は下ぶくれ: 最大幅は顎〜口の高さにあり、上へ絞る
    (0.0883, -0.008, 0.032, 0.031, 0.034, 0.15, "throat"),
    (0.0944, -0.012, 0.038, 0.036, 0.041, 0.30, "jaw"),       # 下顎を少し前へ。頬の最大幅
    (0.1015, -0.0155, 0.037, 0.0375, 0.0415, 0.40, "mouth"),  # 口の高さ。後縁+0.022=項の谷
    (0.1085, -0.015, 0.039, 0.041, 0.040, 0.48, "cheek"),
    (0.1156, -0.0145, 0.042, 0.0455, 0.037, 0.52, "snout_eye"),  # 鼻先(-0.0565)。後頭部最後(+0.031)
    # 頭頂へ: 正面幅は急に絞る(頬張り形)、側面の奥行きは平らに残る
    (0.1226, -0.020, 0.036, 0.038, 0.031, 0.45, "brow"),
    (0.1296, -0.016, 0.030, 0.032, 0.025, 0.32, "forehead"),
    (0.1349, -0.0115, 0.025, 0.0255, 0.016, 0.20, "crown"),
    (0.138, -0.010, 0.014, 0.014, 0.007, 0.10, "top"),
]
...
def build_arms() -> list[bpy.types.Object]:
    ...
    for side in (-1.0, 1.0):
        pts = [
            ...
            # 真下に肩が並び、くびれが隠れる
            Vector((0.025 * side, -0.010, 0.062)),  # 肩(胴の側面に埋まる)
            Vector((0.035 * side, -0.023, 0.048)),  # 上腕
            Vector((0.039 * side, -0.029, 0.035)),  # 肘(最も外)
            Vector((0.032 * side, -0.032, 0.021)),  # 前腕
            Vector((0.026 * side, -0.037, 0.013)),  # 手首(内へ)
            Vector((0.024 * side, -0.044, 0.009)),  # 手
        ]
        arm = C.curve_tube(f"{NAME}_arm{side:+.0f}", pts,
                           [0.014, 0.0135, 0.0125, 0.011, 0.010, 0.0095])
        out.append(arm)
        # 掌: 床に「ぺたっ」と置く
        palm = (0.024 * side, -0.047, 0.006)
        out.append(C.uv_sphere(f"{NAME}_hand{side:+.0f}", palm, 0.011,
                               segments=10, rings=7, scale=(1.1, 1.1, 0.55)))
        # 指3本: 前へ、少し外へ開く
        out += _digits(f"{NAME}_hand{side:+.0f}", (0.024 * side, -0.054, 0.005),
                       forward=(0.15 * side, -1.0, 0.0), spread_axis=(0, 0, 1))
    return out
...
def build_legs() -> list[bpy.types.Object]:
    ...
    out = []
    for side in (-1.0, 1.0):
        thigh_c = (0.032 * side, +0.014, 0.030)
...
def build_tail() -> bpy.types.Object:
    ...
    pts = [
        ...
        Vector((-0.025, +0.110, 0.037)),
        Vector((-0.029, +0.106, 0.050)),  # 渦の頂点
        Vector((-0.031, +0.095, 0.052)),  # 渦の上を前へ
        Vector((-0.031, +0.087, 0.044)),  # 先端(下がって終わる)
    ]
    radii = [0.020, 0.017, 0.013, 0.010, 0.0085, 0.007, 0.006, 0.005, 0.004, 0.0025]
...
# 丸い花弁の連なりになるようにする
FRILL_LOBES = [
    (0.008, 0.009, 0.0035), (0.034, 0.010, 0.007), (0.058, 0.012, 0.013),
    (0.082, 0.012, 0.011), (0.104, 0.011, 0.009), (0.124, 0.010, 0.007),
    (0.142, 0.008, 0.005),
]

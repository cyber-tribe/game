...
# ゲーム表示用の意図的な誇張。実ゲーム距離(俯角48°・FOV46)では、設定画を
# そのまま縮小すると顔の細部が消える。眼裂を太く・まぶた面を広く・口を太く
EYE_DILATE_Z = 3                  # 眼裂の縦膨張(拡大後px)。太さはまぶた面が担う
EYE_SMOOTH = 2.5                  # 眼裂の輪郭を滑らかにする半径(拡大後px)
MOUTH_DILATE_Z = 5                # 口線も同様に太らせる(設定画の線は 2px しかない)
NOSTRIL_DILATE = 2
...
# tools/models/akubitokage_v3.py の目の島の中心・口線の高さと同期させること
# (あちらを動かしたらここも直す。ずれるとデカールが顔からずれる)

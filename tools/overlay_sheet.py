...
def main() -> int:
    ...
    if view in ("side", "back", "self"):
        # **側面と背面は左右を反転する。** モデルのシルエットは-X側にカメラを
        # 置くので画面の右が顔側、設定画の側面図は左が顔側。背面も
        # +Y側から見るので左右が入れ替わる。反転せずに「左(設)/左(モ)」を
        # 並べると、前後(または左右)が入れ替わったまま比べることになる
        # (実測: 靴の前後で差が+74/+83mmと出ていた)

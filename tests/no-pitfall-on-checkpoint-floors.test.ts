...
describe("entities/dungeons.ts: isCheckpointFloor", () => {
  it("地方ダンジョンでは中間階(REGION_CHECKPOINT_FLOOR)だけが該当する", () => {
    expect(isCheckpointFloor(region1, 2)).toBe(false);
    expect(isCheckpointFloor(region1, 3)).toBe(true);
    expect(isCheckpointFloor(region1, 4)).toBe(false);
  });
...
describe("dungeon/gimmicks.ts: チェックポイント階の「おちあなの階」除外", () => {
  it("checkpointFloor=trueでは、多数の抽選でpitfallギミックが選ばれない", () => {
    for (let seed = 1; seed <= 300; seed++) {
      const g = pickFloorGimmick(new Rng(seed), 6, undefined, 10, 1, true);
  ...
  it("checkpointFloor=falseでは従来どおりpitfallが選ばれうる", () => {
    ...
    for (let seed = 1; seed <= 300 && !saw; seed++) {
      saw = pickFloorGimmick(new Rng(seed), 7, undefined, 10, 1, false) === "pitfall";
    }
